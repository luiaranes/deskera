<?php

use Roots\Sage\Extras;

?>
<div class="uk-section uk-section-default">
	<div class="uk-container">
		<div class="uk-grid-match uk-child-width-expand@s no-boxshadow home-banner" uk-grid>
			<div class="banner-content uk-inline">
				<div class="uk-card uk-card-body uk-position-center">
					<h1>Run your business <br> on the cloud,<br> all in one place.</h1>
					<p>An award-winning integrated business platform <br>that helps you run your business.</p>
					<div class="subscribe-form">
						<div uk-form-custom="target: true" class="s">
							<input id="home-banner-input" class="uk-input <?=Extras\getLocation();?>-getStarted-input" type="text" placeholder="your email address">
						</div>
						<button id="home-banner-btn" class="uk-button uk-button-default <?=Extras\getLocation();?>-getStarted-btn">Get Started</button>
					</div>
				</div>
			</div>
			<div class="banner-image">
				<div>
					<img src="<?=get_template_directory_uri();?>/dist/images/home-banner<?=Extras\getLocation();?>.svg" alt="home-banner"/>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="uk-section uk-section-default">
	<div class="uk-container">
		<div class="uk-margin-large-top uk-margin-large-bottom uk-text-center feature-content">
			<h2>A real-time data hub for your team</h2>
			<p>Gain insight into day-to-day business operations in real-time, as Deskera <br> apps come together to help you manage your time and efforts more efficiently.</p>
			<div class="uk-child-width-expand@s uk-text-left uk-margin-large-top" uk-grid>
				<div>
					<div class="uk-card uk-card-body">
						<div class="uk-card-media-top uk-margin-medium-bottom">
							<img src="<?=get_template_directory_uri();?>/dist/images/all-in-one<?=Extras\getLocation();?>.svg" alt=""/>
						</div>
						<div class="uk-card-body uk-padding-remove">
							<h5 class="uk-card-title uk-margin-medium-bottom">Integrated All-in-one Platform</h5>
							<p>Business apps all in one place, so that you can manage every aspect of your business with ease.</p>
						</div>
					</div>
				</div>
				<div>
					<div class="uk-card uk-card-body">
						<div class="uk-card-media-top uk-margin-medium-bottom">
							<img src="<?=get_template_directory_uri();?>/dist/images/quick-to-deploy<?=Extras\getLocation();?>.svg" alt=""/>
						</div>
						<div class="uk-card-body uk-padding-remove">
							<h5 class="uk-card-title uk-margin-medium-bottom">Quick to deploy</h5>
							<p>Avoid long waits and downtime as Deskera apps fit into your workplace seamlessly and quickly.</p>
						</div>
					</div>
				</div>
				<div>
					<div class="uk-card uk-card-body">
						<div class="uk-card-media-top uk-margin-medium-bottom">
							<img src="<?=get_template_directory_uri();?>/dist/images/cloud-based-software<?=Extras\getLocation();?>.svg" alt=""/>
						</div>
						<div class="uk-card-body uk-padding-remove">
							<h5 class="uk-card-title uk-margin-medium-bottom">Cloud-based software</h5>
							<p>Access the information anytime, anywhere, on any connected device, so you can work on the go.</p>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="uk-section uk-section-default">
	<div class="uk-container">
		<div class="uk-margin-large-top uk-text-center organization-content">
			<h2>Organizations love us</h2>
			<p>Hear from customers that use Deskera <br> to maximize their efficiency in the business.</p>
			<div class="uk-flex uk-flex-center uk-margin-large-top">
				<div class="uk-card uk-card-body uk-margin-large-right uk-padding-small">
					<div class="uk-card-media-top uk-margin-medium-bottom">
						<img src="<?=get_template_directory_uri();?>/dist/images/senwan-group.svg" alt=""/>
					</div>
					<div class="uk-card-body uk-padding-remove">
						<h5 class="uk-card-title uk-margin-small-bottom">Revolutionizing <br> the way <br> business is run</h5>
					</div>
					<div class="uk-text-right">
						<a class="#">
							<img src="<?=get_template_directory_uri();?>/dist/images/arrow-right<?=Extras\getLocation();?>.svg"/>
						</a>
					</div>
				</div>
				<div class="uk-card uk-card-body uk-margin-large-right uk-padding-small">
					<div class="uk-card-media-top uk-margin-medium-bottom">
						<img src="<?=get_template_directory_uri();?>/dist/images/baker-tilly.svg" alt=""/>
					</div>
					<div class="uk-card-body uk-padding-remove">
						<h5 class="uk-card-title uk-margin-small-bottom">Making workflow easier for everyone</h5>
					</div>
					<div class="uk-text-right">
						<a class="#">
							<img src="<?=get_template_directory_uri();?>/dist/images/arrow-right<?=Extras\getLocation();?>.svg"/>
						</a>
					</div>
				</div>
				<div class="uk-card uk-card-body uk-margin-large-right uk-padding-small">
					<div class="uk-card-media-top uk-margin-medium-bottom">
						<img src="<?=get_template_directory_uri();?>/dist/images/felda-iffco.svg" alt=""/>
					</div>
					<div class="uk-card-body uk-padding-remove">
						<h5 class="uk-card-title uk-margin-small-bottom">Focusing on the important things</h5>
					</div>
					<div class="uk-text-right">
						<a class="#">
							<img src="<?=get_template_directory_uri();?>/dist/images/arrow-right<?=Extras\getLocation();?>.svg"/>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="uk-section uk-section-default">
	<div class="uk-container">
		<div class="uk-text-center uk-margin-large-bottom client-content">
			<div class="uk-flex uk-flex-center" uk-grid uk-height-match="target: > .uk-card">
				<div class="uk-card uk-card-body uk-flex uk-flex-center uk-padding-remove-horizontal">
					<img src="<?=get_template_directory_uri();?>/dist/images/sushi-tei.svg"/>
				</div>
				<div class="uk-card uk-card-body uk-flex uk-flex-center uk-padding-remove-horizontal">
					<img src="<?=get_template_directory_uri();?>/dist/images/mitsubishi-electric.svg"/>
				</div>
				<div class="uk-card uk-card-body uk-flex uk-flex-center uk-padding-remove-horizontal">
					<img src="<?=get_template_directory_uri();?>/dist/images/sicci-logo.svg"/>
				</div>
				<div class="uk-card uk-card-body uk-flex uk-flex-center uk-padding-remove-horizontal">
					<img src="<?=get_template_directory_uri();?>/dist/images/telkomsel.svg"/>
				</div>
				<div class="uk-card uk-card-body uk-flex uk-flex-center uk-padding-remove-horizontal">
					<img src="<?=get_template_directory_uri();?>/dist/images/gic.svg"/>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="subcribe-content uk-padding-large <?=Extras\getLocation();?>-bg">
	<div class="uk-container">
		<div class="uk-flex-middle" uk-grid>
			<div class="uk-width-1-2@l">
				<p>Let us free up your time for you. Free 14-day trial.</p>
			</div>
			<div class="uk-width-1-2@l">
				<div class="subscribe-form">
					<div uk-form-custom="target: true" class="s">
						<input class="uk-input" type="text" placeholder="your email address">
					</div>
					<button class="uk-button uk-button-default <?=Extras\getLocation();?>-color">Get Started</button>
				</div>
			</div>
		</div>
	</div>
</div>